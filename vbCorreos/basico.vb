...
Module basico
    ...
    Sub Main()
        ...
        Else
            ...
            If Not optimizar_correo Then
                cadSQL = "SELECT *, 1 as cuenta  FROM sigma.mensajes WHERE canal = 3 AND estatus = 'A' ORDER BY prioridad DESC"
            ElseIf mantenerPrioridad Then
                cadSQL = "SELECT prioridad, canal, destino, count(*) as cuenta FROM sigma.mensajes WHERE canal = 3 AND estatus = 'A' GROUP BY prioridad, canal, destino ORDER BY prioridad DESC"
            Else
                cadSQL = "SELECT canal, destino, count(*) as cuenta FROM sigma.mensajes WHERE canal = 3 AND estatus = 'A' GROUP BY canal, destino ORDER BY prioridad DESC"
            End If
                    ...
                    For Each elmensaje In mensajesDS.Tables(0).Rows
                        ...
                        If optimizar_correo Then
                            If elmensaje!cuenta = 1 Then
                                ...
                                cadSQL = "SELECT mensaje FROM sigma.mensajes WHERE canal = 3 AND destino = '" & elmensaje!destino & "' AND estatus = 'A' " & fPrioridad & " "
                                ...
                                If correo_titulo_falla Then
                                    tituloMensaje = Microsoft.VisualBasic.Strings.Left(eMensaje, Microsoft.VisualBasic.Strings.InStr(eMensaje, vbCrLf) - 1)
                                End If
                                If tituloMensaje.Length = 0 Then tituloMensaje = correo_titulo
                                If tituloMensaje.Length = 0 Then tituloMensaje = "Este es un mensaje del Monitor de Tenneco"

                            Else
                                eMensaje = ""
                                tituloMensaje = tituloMensaje & ": Tiene " & elmensaje!cuenta & " mensaje(s) por atender"
                                cadSQL = "SELECT fecha, mensaje FROM sigma.mensajes WHERE canal = 3 AND destino = '" & elmensaje!destino & "' AND estatus = 'A'"
                        ...
                        If optimizar_correo Then
                            ...
                            End If
                            'Doble cic en el mensaje
                            regsAfectados = consultaACT("UPDATE sigma.mensajes SET estatus = 'Z' WHERE canal = 3 AND destino = '" & elmensaje!destino & "' AND estatus = 'A' " & fPrioridad)
                        Else
    ...
    Private Sub agregarLOG(cadena As String, tipo As Integer, reporte As Integer, Optional aplicacion As Integer = 1)
        ...
        Dim regsAfectados = consultaACT("INSERT INTO sigma.log (aplicacion, tipo, reporte, texto) VALUES (20, " & tipo & ", " & reporte & ", '" & Microsoft.VisualBasic.Strings.Left(cadena, 250) & "')")
